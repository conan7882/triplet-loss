...
def prepare_query_gallery(TripletDataFlow, ChildDataFlow, data_dir, batch_dict_name, shuffle=True, pf=dfutil.identity,
                          query_ratio=0.3):
    ...
    for class_id, im_list in enumerate(triplet_dataflow.im_list):
        ...
        # query_im_list.append(list(im_list[:query_sample]))
        # query_label_list.append(list(im_list[:query_sample]))
        # gallery_im_list.append(list(im_list[query_sample:]))
        # gallery_label_list.append(list(im_list[query_sample:]))
